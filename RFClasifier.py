...
def get_featurevector(data):
    ...
    featurevecs[1] = 0#averaged[1]
    featurevecs[2] = 0#averaged[2]
    featurevecs[3] = 0#averaged[3]
    featurevecs[4] = 0#summed[3] / 10
    _, elevation = pol2cart(data[:,0], data[:,4])
    featurevecs[5] = (np.percentile(elevation, 95) + np.percentile(elevation, 90))/2
    featurevecs[6] = 0#np.mean(data[:,3]) / ((1/(averaged[0]/1400) +130)) if averaged[0] > 6 else (np.mean(data[:,3])/360)
...
#clf = RandomForestClassifier(max_depth=5, criterion="entropy", random_state=0,n_estimators=50)
#clf = nn.MLPClassifier((100,100,100), max_iter=200, alpha=0.10)
...
plot_confusion_matrix(val_labels, validation_pred, ['adult', 'bicycle', 'child','unlabeled'])
plt.show()
